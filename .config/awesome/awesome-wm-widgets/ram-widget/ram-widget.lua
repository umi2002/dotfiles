...
local function worker( user_args )
    ...
    local color_used = theme.border_focus
    local color_free = theme.border_normal
    local color_buf = theme.border_focus
    ...
    watch(
        "bash -c \"LANGUAGE=en_US.UTF-8 free | grep -z Mem.*Swap.*\"", timeout,
        function( widget, stdout )
            ...
            if popup.visible then
                popup:get_widget().data_list = {
                    {
                        "used " .. getPercentage( used + used_swap ),
                        used + used_swap,
                    },
                    {
                        "free " .. getPercentage( free + free_swap ),
                        free + free_swap,
                    },
            ...
            awful.button(
                {}, 1, function()
                    popup:get_widget().data_list = {
                        ...
                        {
                            "buff_cache " .. getPercentage( buff_cache ),
                            buff_cache,
                        },
                    }
